            ...
            if export_artifact_route is not None:
                ...
                try:
                    ...
                    if artifact_kind == "review-completion":
                        ...
                        return
                    if artifact_kind == "acceptance-report":
                        self._send_json(
                            control_plane.runtime_export_package_acceptance_report(
                                package_id
                            )
                        )
                        return
            ...
            if export_artifact_route is not None:
                ...
                if artifact_kind == "route-comparison-review-report":
                    try:
                        ...
                        self.send_error(400, str(exc))
                    return
                if artifact_kind == "service-trace-comparison-review-report":
                    try:
                        payload = self._read_json_body()
                        if not isinstance(payload, dict):
                            raise ValueError("request body must be a JSON object")
...
def _runtime_export_package_route(
    ...
    if len(parts) == 2 and parts[1] == "acceptance-report":
        return parts[0], "acceptance-report", None
